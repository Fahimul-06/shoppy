...
export async function fetchSellerShop(id: string): Promise<{ seller: any; products: Product[]; categories: string[] }> {
  return api.get<{ seller: any; products: Product[]; categories: string[] }>(`/shops/${encodeURIComponent(id)}`);
}
