...
export default function AdminLoginPage() {
  ...
  const login = async () => {
    ...
    try {
      const res = await api.post<{ token: string; user: any }>('/admin/login', { email, password });
  ...
  return <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
    <div className="w-full max-w-md bg-white rounded-3xl p-7 shadow-2xl">
      ...
      <p className="text-sm text-gray-500 mb-6">Protected MongoDB admin dashboard.</p>
      <div className="space-y-3">
        <div className="relative"><Mail size={16} className="absolute left-3.5 top-3.5 text-gray-400"/><input className="w-full pl-10 pr-4 py-3 rounded-xl border text-sm" value={email} onChange={(e)=>setEmail(e.target.value)} placeholder="Email" /></div>
