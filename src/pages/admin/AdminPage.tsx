...
import { LayoutDashboard, Users, Package, ShoppingBag, Tag, LogOut, Shield, UserCog } from 'lucide-react';
...
type Tab = 'dashboard' | 'users' | 'sellers' | 'products' | 'orders' | 'promos';
type Stats = { users:number; customers:number; admins:number; sellers:number; pendingSellers:number; approvedSellers:number; products:number; activeProducts:number; orders:number; revenue:number; activePromos:number };
...
export default function AdminPage() {
  ...
  const nav = [
    ['dashboard','Dashboard',LayoutDashboard], ['users','Users',UserCog], ['sellers','Sellers',Users], ['products','Products',Package], ['orders','Orders',ShoppingBag], ['promos','Promo Codes',Tag],
  ] as const;
  ...
  return <div className="min-h-screen bg-slate-50 flex">
    ...
    <main className="flex-1 min-w-0"><header className="bg-white border-b border-gray-100 px-8 py-4"><h1 className="font-extrabold text-gray-900 text-lg capitalize">{tab === 'promos' ? 'Promo Codes' : tab}</h1><p className="text-xs text-gray-400">Render + MongoDB dashboard</p></header><div className="p-8">
      {tab==='dashboard' && <div className="space-y-6"><div className="grid grid-cols-2 xl:grid-cols-3 gap-4"><Stat label="Total Users" value={stats?.users ?? '-'} onClick={()=>setTab('users')} /><Stat label="Customers" value={stats?.customers ?? '-'} onClick={()=>setTab('users')} /><Stat label="Total Sellers" value={stats?.sellers ?? '-'} onClick={()=>setTab('sellers')} /><Stat label="Pending Sellers" value={stats?.pendingSellers ?? '-'} onClick={()=>setTab('sellers')} /><Stat label="Products" value={stats?.products ?? '-'} onClick={()=>setTab('products')} /><Stat label="Active Products" value={stats?.activeProducts ?? '-'} onClick={()=>setTab('products')} /><Stat label="Orders" value={stats?.orders ?? '-'} onClick={()=>setTab('orders')} /><Stat label="Paid Revenue" value={`৳${(stats?.revenue ?? 0).toLocaleString()}`} /><Stat label="Active Promos" value={stats?.activePromos ?? '-'} onClick={()=>setTab('promos')} /></div><div className="bg-blue-50 border border-blue-100 rounded-2xl p-5 text-sm text-blue-700">Backend is now Express + MongoDB. Supabase has been removed.</div></div>}
      {tab==='users' && <AdminUsersTab />}{tab==='sellers' && <AdminSellersTab />}{tab==='products' && <AdminProductsTab />}{tab==='orders' && <AdminOrdersTab />}{tab==='promos' && <AdminPromoCodesTab />}
    </div></main>
  </div>;
}
