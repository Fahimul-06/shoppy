    ...
    <main className="max-w-5xl mx-auto px-4 py-6">
      ...
      {loading ? <div className="bg-white border rounded-2xl p-6 text-sm text-gray-500">Loading assigned orders...</div> : <div className="space-y-4">{orders.map((o)=> {
        ...
        return <div key={o.id} className="bg-white border rounded-2xl p-5">
          ...
          <div className="mt-4 flex gap-2">{o.status !== 'delivered' ? <button onClick={()=>updateStatus(o.id, 'delivered')} className="px-4 py-2 rounded-xl bg-green-600 text-white font-black text-sm">Mark Delivered</button> : <span className="px-4 py-2 rounded-xl bg-green-50 text-green-700 border border-green-200 font-black text-sm">Delivered</span>}</div>
